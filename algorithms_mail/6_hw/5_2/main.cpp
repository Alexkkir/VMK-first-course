        ...
        for (unsigned long long i = 0; i < n + 1 - len; i++) {
            ...
            for (int j = i + 1; j < n + 1 - len; j++) {
                if (hashes[j].x == hashes[i].x) {
                    ...
                    if (same == true) {
                        count++;
                        hashes[j].in_use = false;
                    }
                }
            }

            hashes[i].in_use = false;
