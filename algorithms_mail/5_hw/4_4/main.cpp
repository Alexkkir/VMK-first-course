...
typedef struct avl_tree_with_parent_link {
    node *root;
    int size;
    ...
    static int comparator(key_type x, key_type y) {
        // ��� �����
        return (x > y);
        // ��� �����
    ...
    static node* rotate_right(node* p) // правый поворот вокруг p
    {
        node* q = p->left;
        p->left = q->right;
        q->right = p;
    ...
    static node* rotate_left(node* q) // левый поворот вокруг q
    {
        node* p = q->right;
        q->right = p->left;
        p->left = q;
    ...
    void rotated_print_node(node *node) {
        rotated_print_node_(root, 0, node);
    }

    void rotated_print_node_(node* p, int n_level, node *node){
        if (!p) return;
        rotated_print_node_(p->right, n_level + 1, node);
        for ( int i = 0; i < n_level; i++, printf("\t") );
        if (p == node) printf("*");
        printf("%d\n", p->key);
        rotated_print_node_(p->left, n_level + 1, node);
    }

    static bool is_leaf(node *p) {
    ...
    void find_by_index_(node *p, int index, int &curr, int &ans, bool &in_process) {
...
int main(void) {
    avl_tree_with_parent_link tree;
    ...
    for (int i = 0; i < n; i++) {
        ...
        switch (op) {
            ...
            case 3:
                ...
                break;
            case 4:
                tree.rotated_print_node(median);
                break;
            default:
                break;
        }
