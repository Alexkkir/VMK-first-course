...
int is_first_bigger_than_second(int x, int y) {
  // для чисел
  return (x > y);
  // для строк
...
node* remove_node(node* p, int k) // удаление ключа k из дерева p
{
	if( !p ) return NULL;
	if( is_first_bigger_than_second(p->key, k) ) {
...
node* find(node* p, int k, int* p_times)
{
  if( p ) {
    if( (*p_times)++ && is_first_bigger_than_second(k, p->key) ){
      return find(p->right, k, p_times);
    }
    else if(is_first_bigger_than_second(p->key, k) ){
      return find(p->left, k, p_times);
    }
    else {
      return p;
    }
...
int main(void) {
  ...
  int nums[15] = {63, 71, 2, 41, 49, 20, 31, 54, 33, 35, 40, 50, 61, 3, 87};
  int n = 15;
